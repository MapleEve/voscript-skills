...
本技能包同步 VoScript `v0.7.5` 的公开行为与排障口径。对调用方来说，核心
HTTP 工作流仍是提交任务、轮询、取结果、管理声纹和导出文件；`0.7.5`
主要补齐服务端 GPU 模型生命周期、模型缓存、本地快照加载和运行时默认值说明，
不要求客户端改变既有 API 调用方式。
