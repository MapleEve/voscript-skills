...
本技能包同步 VoScript `v0.7.2` 的当前发布叙事与行为：architecture foundation、
stability hardening。对调用方来说，核心 HTTP
工作流仍是提交任务、轮询、取结果、管理声纹和导出文件；内部的
`pipeline / providers / infra / voiceprints` 分层不要求用户改变调用方式。
...
`${SKILL_PATH}/references/remote-debugging.md`。每次远端调试前，先让用户在本机终端执行
`ssh -fN ai`（内网）或 `ssh -fN ai-wan`（外网），再由代理使用
`ssh ai '<cmd>'` 或 `ssh ai-wan '<cmd>'` 执行只读确认、部署/重启、smoke、日志监控与收尾。
如果代理工具沙箱无法连通但用户本机终端可连通，不得把沙箱网络错误当成远端不可达；
必须改走用户本机终端可用的 SSH alias 路线，或停止并说明需要本机终端路线。远端部署前
必须先检查远端 worktree；如有本地改动，先保存 status、patch 和 stash，再对齐 release
分支/提交。部署后必须等待服务健康，确认 `/healthz`、`/openapi.json` 版本，启动持续日志
监控，并在客户端入口出现 HTTP-to-HTTPS 端口错误时改用 `https://`。
...
9. **容器日志**：`docker logs voscript` 查看详细栈回溯
